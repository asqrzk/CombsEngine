...
pub(crate) fn create_server(setup: WgpuSetup, options: RuntimeOptions) -> WgpuServer {
    ...
    // On the web this must stay off: wgpu's WebGPU backend advertises
    // subgroup support it can never deliver (AdapterInfo's plane sizes are
    // hardcoded constants, and `requiredFeatures` never includes
    // `subgroups`), so kernels that emit `enable subgroups;` fail WGSL
    // validation on a device without the feature — and a failed pipeline
    // dispatches as a silent no-op, which surfaces as all-zero outputs,
    // not as an error.
    #[cfg(not(any(all(target_os = "macos", feature = "msl"), target_family = "wasm")))]
    {
